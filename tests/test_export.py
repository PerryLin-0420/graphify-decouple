...
def test_hyperedge_perimeter_uses_covariance_ellipse_not_member_order():
    """The hyperedge region must be a covariance-ellipse fit, not a polygon
    traced through `h.nodes` array order — tracing raw member order
    self-intersects whenever the layout does not place members in angular
    order, so `fill()` paints crossed wedges instead of one region. The
    covariance ellipse is order-independent by construction (it derives only
    from the point scatter's mean and covariance matrix), so it never has
    this failure mode."""
    from graphify.exporters.html import _hyperedge_script
    script = _hyperedge_script("[]")
    assert "ctx.ellipse(cx, cy, rx, ry, angle, 0, 2 * Math.PI)" in script
    assert "varX += dx * dx; varY += dy * dy; covXY += dx * dy;" in script
    # the region must never trace member positions as a raw-order polygon path
    assert "expanded.slice(1).forEach(p => ctx.lineTo(p.x, p.y));" not in script
