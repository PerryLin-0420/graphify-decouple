...
<p align="center">
  <em>DECOUPLE.html في تشغيل حقيقي — النقر على فئة مقترحة يُظهر بالتحديد أي فئة أخرى تشاركها الحالة، وما هو المُشترك بالضبط.</em>
</p>

الصفحة نفسها ترسم أيضًا عملية التقسيم ذاتها. تبديل **Preview decoupled view** يستبدل توابع الفئة الإلهية الخاصة بها بالفئات المقترحة ويعيد توجيه الحواف في مكانها — تغيير التوصيل نفسه، لا مخططًا أُعيد رسمه:

| قبل — الفئة الإلهية اليوم | بعد — Preview decoupled view |
| --- | --- |
| <img src="../decouple-before.png" alt="DECOUPLE.html قبل التبديل: عقدة محورية واحدة MainWindow تتوزع حولها توابعها الخاصة" width="440"> | <img src="../decouple-after.png" alt="DECOUPLE.html بعد التبديل: العقدة نفسها مختزلة إلى 5 فئات مقترحة على شكل معينات، حواف خضراء متقطعة تُظهر أي التوابع استُخرج إلى كل منها، وحواف حمراء تُظهر حالة النسخة المشتركة بين اثنتين منها" width="440"> |
| عقدة واحدة تضم 47 من توابعها الخاصة، كل واحد منها لا يمكن الوصول إليه إلا عبر الفئة. | الفئات المقترحة. الأخضر المتقطع = ما استُخرج إلى كل منها؛ الأحمر = حالة النسخة التي ما زالت اثنتان منها تتشاركانها، وهي بالضبط ما يحسم بين `split` و `keep_as_is`. تُرسم فقط المرشحات التي تتجاوز عتبة المخاطرة — هنا 5 من 6، ولهذا يبقى تابع واحد بلا معين يحط عليه. |
